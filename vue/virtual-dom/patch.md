...
function patch (oldVnode, vnode, hydrating, removeOnly) {
    // 如果vnode不存在
    if (isUndef(vnode)) {
      // 但是旧节点存在，直接调用invokeDestroyHook销毁旧节点
    ...
    if (isUndef(oldVnode)) {
      ...
      // 判断老节点的nodeType（元素的节点类型）,并且是同时存在，而且是相同的节点时调用patchVnode对比子节点
...
#### 6. 当判断到newStartIdx > newEndIdx时，新节点已经遍历完了，旧节点还没有遍历完，说明老节点有多余的节点，会调用removeVnodes函数从文档中删除真实dom




#### 7. 反之，当判断到oldStartIdx > oldEndIdx时，说明旧节点已经遍历完了，但是新节点还没有遍历完，说明这些是新增的节点，会调用createElm创建新的dom节点，并且调用addVnode插入到真实dom中，
![](./diff-6.jpg)
...
// 创建create', 'activate', 'update', 'remove', 'destroy声明钩子
export function createPatchFunction (backend) {
  ...
  //当vnode对象全部创建完之后，就调用依次调用这个队列的insert钩子
  function createElm (
    vnode,
    insertedVnodeQueue,
    parentElm,
